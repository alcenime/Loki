(function () {
  ...
  const CSS = `
    ...
    .top-spacer { flex: 1; }
    .currency-group { display: flex; align-items: center; gap: 6px; }
    .currency-pill {
  ...
  // ─── NAV ITEMS CONFIG ───
  const NAV_ITEMS = [
    { key: 'home',        icon: 'homes.png',       label: 'Home',        href: 'game.html' },
  ...
  function buildTopbar() {
    ...
    <div class="topbar">
      ...
      <div class="top-spacer"></div>
      <div class="connect-wallet-btn" onclick="document.getElementById('coming-soon-popup') && (document.getElementById('coming-soon-popup').style.display='flex')">
  ...
  window.loadSidebar = function (activePage) {
    ...
    // Inject topbar — cari .topbar yang ada, replace isinya; atau inject ke center-col
    const existingTopbar = document.querySelector('.topbar');
    const topbarTemp = document.createElement('div');
    topbarTemp.innerHTML = buildTopbar();
    const newTopbar = topbarTemp.firstElementChild;

    if (existingTopbar) {
      existingTopbar.replaceWith(newTopbar);
    } else {
      // fallback: prepend ke .center-col kalau ada
