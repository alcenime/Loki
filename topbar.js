(function () {
  ...
  const CSS = `
    ...
    .tb-spacer { flex: 1; }
    .tb-logo { height: 40px; object-fit: contain; flex-shrink: 0; }
    ...
    .tb-energy-pill .tb-plus {
      width: 20px; height: 20px; border-radius: 50%;
      background: rgba(160,96,255,0.8); color: white;
      font-size: 13px; font-weight: 900;
      display: flex; align-items: center; justify-content: center;
      cursor: pointer; flex-shrink: 0; margin-left: 2px;
    }
    ...
    .tb-wallet-btn:hover { border-color: #e8c96a; }
  `;
  ...
  function buildTopbar(options) {
    ...
    const showLogo   = options && options.showLogo   === false ? false : true;
    const logoSrc    = (options && options.logoSrc) || 'assets/dashboard/lokilogo.png';

    const logoHTML = showLogo
      ? `<img src="${logoSrc}" alt="Loki" class="tb-logo" onerror="this.style.display='none'">`
      : '';
    ...
    const walletHTML = showWallet ? `
      <div class="tb-wallet-btn" onclick="document.getElementById('coming-soon-popup') && (document.getElementById('coming-soon-popup').style.display='flex')">
        <img src="assets/dashboard/icons/wallet.png" alt="wallet" style="width:16px;height:16px;object-fit:contain;" onerror="this.style.display='none'">
    ...
    <div class="topbar">
      ...
      <div class="tb-currency-group">
        <div class="tb-currency-pill">
          <img src="assets/buy/wolfgold.png" style="width:18px;height:18px;object-fit:contain;flex-shrink:0;" onerror="this.style.display='none'"/>
          <span>0</span>
          <div class="tb-plus">+</div>
        </div>
        <div class="tb-currency-pill">
          <img src="assets/buy/wolfcoin.png" style="width:18px;height:18px;object-fit:contain;flex-shrink:0;" onerror="this.style.display='none'"/>
          <span>0</span>
          <div class="tb-plus">+</div>
        </div>
        <div class="tb-energy-pill">
          <img src="assets/dashboard/icons/energy.png" style="width:18px;height:18px;object-fit:contain;flex-shrink:0;" onerror="this.style.display='none'"/>
          <span>0/100</span>
          <div class="tb-plus">+</div>
        </div>
      </div>
